...
(function () {
    ...
    function setAuthorizationHeaderTemplate($localStorage, scConnection, $httpDefaults) {
        if ($localStorage.scStore && $localStorage.scStore.token) {
            ...
            if (scConnection.authenticationMethod === "basic" && $localStorage.scStore.type === "basic") {
    ...
    angular.module('sociocortex').service('scAuth', ['$localStorage', '$q', '$http', 'scUtil', 'scConnection', function scAuthentication($localStorage, $q, $http, scUtil, scConnection) {
        return {
